...
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__),
                                             os.path.pardir)))
# from easy_context.dist_flash_attn.lightseq_async_attn import attention as lightseq_attn # [NOTE]: Disable lightseq
...
def run_all_inter_attn(args, ncclcomm_global, gloo_global_group):
    ...
    baseline_funcs = [
        ...
        # deepspeed_ulysses_attn_func,    # [NOTE]: TODO
    ]
    ...
    CLUSTER_NAME, PLATFORM = os.environ.get('CLUSTER_NAME', None), os.environ.get('PLATFORM', None)
    INTER_COMP_FILE_NAME = f'./prof_data/{CLUSTER_NAME}/wrapper_intra_SP={local_size}_all.log'
    
    # get_comp_map_key
    if os.path.exists(INTER_COMP_FILE_NAME):
        inter_comp_profile_map_raw = Inter_Comp_Profile_Map(select_best_schedule_in_node_profile_data(INTER_COMP_FILE_NAME, local_size))
    else:
        inter_comp_profile_map_raw = None
            ...
            for Nh in Nhs:
                for Sqkv in Sqkvs: # S per GPU
                    ...
                    if causal:  # For causal !!!
                        ...
                        ablation_levels = [0, 1, 0, 2]
                        # ablation_suffixes = ['_fused']      # for torch.profiler
                        # ablation_suffixes = ['']            # for SP0 = 1
                        # ablation_suffixes = []
                        
                        
                        par_dir = f'{os.path.dirname(__file__)}/search_algo/execution_plans/{CLUSTER_NAME}/{PLATFORM}/inter_SP{node_num}_fob={fob}'
                        plan_paths = []
                        ...
                        for plan_type in plan_types:
                            for plan_path, ablation_label in zip(plan_paths, ablation_levels):
                                # load inter execution plans
                                with open(plan_path, 'rb') as fin:
                                    inter_execution_plan = pickle.load(fin)
                                inter_comp_profile_map = copy.copy(inter_comp_profile_map_raw)
                                inter_comp_profile_map.change_current_level(ablation_label)
                                # print_rank_0(f'inter_comp_profile_map: {inter_comp_profile_map.cur_level}')
                                # print_rank_0(f'inter_comp_profile_map_raw: {inter_comp_profile_map_raw.cur_level}')
                        ...
                        par_dir = f'{os.path.dirname(__file__)}/search_algo/execution_plans/{CLUSTER_NAME}/{PLATFORM}/intra_SP{node_num}_fob={fob}'
                        YXs = [(y, node_num // y) for y in range(1, node_num + 1) if node_num % y == 0]
                        print_rank_0(f'YXs: {YXs}')
                        for plan_type in plan_types:
                            # non fused
                            inter_comp_profile_map = copy.copy(inter_comp_profile_map_raw)
                            inter_comp_profile_map.change_current_level(0)
                            for yx in YXs:
                            ...
                            if fob == 0 or Nh == 1:
                                inter_comp_profile_map = copy.copy(inter_comp_profile_map_raw)
                                inter_comp_profile_map.change_current_level(1)
                                for yx in YXs:
                                # for yx in YXs[: 1]: # qo
...
# da_config: SP=(1,8),Sg=(16384,16384),S=(131072,131072),Nh=(32,32),bs=1,D=128,causal=False,hierarchy=1
# # orchestrated_attn_func
# mfu: 128.152 Tflops/s, hfu: 160.189 Tflops/s, 1.821 iter/s, 5.491e-01 s/iter, (1.208, 0.000, 2.196) sec
# mfu: 136.896 Tflops/s, hfu: 171.12 Tflops/s, 1.945 iter/s, 5.140e-01 s/iter, (9.182, 0.001, 2.056) sec
# mfu: 138.296 Tflops/s, hfu: 172.87 Tflops/s, 1.965 iter/s, 5.088e-01 s/iter, (16.679, 0.000, 2.035) sec
# mfu: 122.401 Tflops/s, hfu: 153.002 Tflops/s, 1.739 iter/s, 5.749e-01 s/iter, (24.340, 0.000, 2.300) sec    
# da_config: SP=(1,8),Sg=(32768,32768),S=(262144,262144),Nh=(32,32),bs=1,D=128,causal=False,hierarchy=1
# # orchestrated_attn_func
# mfu: 193.843 Tflops/s, hfu: 193.843 Tflops/s, 1.377 iter/s, 7.260e-01 s/iter, (1.542, 0.000, 2.904) sec
# mfu: 195.578 Tflops/s, hfu: 195.578 Tflops/s, 1.390 iter/s, 7.196e-01 s/iter, (12.201, 0.001, 2.878) sec
# mfu: 195.961 Tflops/s, hfu: 195.961 Tflops/s, 1.392 iter/s, 7.182e-01 s/iter, (22.667, 0.000, 2.873) sec
# mfu: 214.822 Tflops/s, hfu: 214.822 Tflops/s, 1.526 iter/s, 6.551e-01 s/iter, (32.994, 0.000, 2.621) sec
    ...
    for fob in fobs:
        ...
        for causal in causals:
            ...
            if not causal:
                ...
                for plan_path in plan_paths:
                # for plan_path in plan_paths[:1]:    # [DEBUG]
                    with open(plan_path, 'rb') as fin:
                ...
                for Sq in Sqs: # S per GPU
                    for Skv in Skvs: # S per GPU
                        ...
                        if causal:
                            ...
                            par_dir = f'{os.path.dirname(__file__)}/search_algo/execution_plans/{CLUSTER_NAME}/{PLATFORM}/intra_SP{local_size}_fob={fob}_causal=True'
                            plan_name_prefix = da_config.get_plan_name(fob=fob)
                            # load intra plans
                        ...
                        if causal:
                        # if fob == 0:
                        # if True:    # No backward_only ???
                            for f in baseline_funcs:
                                benchmark(args, f, da_config, tensor_buf, gloo_global_group, fob=fob, log=True)
                        else:
                            f = ring_flash_attn_func
                            benchmark(args, f, da_config, tensor_buf, gloo_global_group, fob=fob, log=True)
...
def profile_all_intra_BSA(args, ncclcomm_global, gloo_global_group):
    global PROC_INFO
    # baseline_funcs = [
    #     ring_flash_attn_func,
    #     zigzag_ring_flash_attn_func,      # baseline
