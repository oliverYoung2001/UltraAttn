...
# def benchmark_orchestrate_old_for_intra(args, raw_f, da_config: Dist_Attn_Config, tensor_buf: torch.Tensor, warmup=11, num_iter=20, log=True, 
...
def benchmark_orchestrate(args, raw_f, da_config: Dist_Attn_Config, tensor_buf: torch.Tensor, warmup=11, num_iter=20, log=True, 
    ...
    for exp_config in exp_configs:
        ...
        inter_comp_plans = {}   # batch_degrees -> plan
            ...
            if isinstance(kernel, Comp_Kernel):
                ...
                if batch_degrees not in inter_comp_plans.keys():
                    if inter_comp_profile_map is not None:
                        ...
                        intra_full_attn_config = (da_config.SP[1], 1, da_config.Nh[0] == 1, - 0.0)
                    ...
                    if intra_full_attn_config[2] == 0:  # not fused, to load intra execution plan
                        ...
                        inter_comp_plans[batch_degrees] = intra_full_attn_config
        ...
        for batch_degrees, intra_execution_plan in inter_comp_plans.items():
            if not isinstance(intra_execution_plan, Execution_Plan):
                continue
            if len(streams['intra']) < intra_execution_plan.stream_num:
                priorities = [0, - 1, - 2]
                priorities = [0] * intra_execution_plan.stream_num
                for _ in range(len(streams['intra']), intra_execution_plan.stream_num):
        ...
        for kernel in inter_execution_plan.valid_kernels:
            if isinstance(kernel, Comm_Kernel):
                ...
                # print_rank_0(f'inter nccl key: {key}')
                # print(f'rank{rank}, inter nccl key: {key}', flush=True)
                    # new_group = cpu_group_dict[tuple(sorted(key))]
                if rank in key:
                    if key not in ncclcomm_dict.keys():
            ...
            # For Intra
        for batch_degrees, intra_execution_plan in inter_comp_plans.items():
            if isinstance(intra_execution_plan, Execution_Plan):    # non-fused intra execution plan
                for kernel in intra_execution_plan.valid_kernels:
                    if isinstance(kernel, Comm_Kernel):
                        ...
                        if rank in key:
                            if key not in ncclcomm_dict.keys():
                                ncclcomm_dict[key] = PyNcclCommunicator(global_group, ranks=key, device=torch.cuda.current_device())
                            kernel.ncclcomm = ncclcomm_dict[key]
            else:       # fused intra execution plan
                assert isinstance(intra_execution_plan, tuple)
...
def benchmark_fused(args, raw_f, da_config: Dist_Attn_Config, tensor_buf, warmup=11, num_iter=20, log=True, 
    ...
    def create_inputs(fob: int, X: int, Y: int):
        # qkvdo = tensor_buf[: 4 * batch_size * seqlen * nheads * d].view(4 * batch_size, seqlen, nheads, d)
        # # qkv.requires_grad = True
        # q, k, v, do = qkvdo.chunk(4, dim=0)
        # D_buf = tensor_buf[4 * batch_size * seqlen * nheads * d: ]
        ...
        inputs = {
            "inp_row": ir_class.from_da_config_with_buf(da_config=da_config, buf=buf_dict['ir'][ir_nelems * cur_x_id: ir_nelems * (cur_x_id + 1)]), 
            "inp_col": ic_class.from_da_config_with_buf(da_config=da_config, buf=buf_dict['ic'][ic_nelems * cur_y_id: ic_nelems * (cur_y_id + 1)]),
            "dropout_p": dropout_p,
            "causal": None,
            "deterministic": deterministic,
            "softmax_scale": d ** (-0.5),
    ...
    for plan_path in plan_paths:
        ...
        with open(plan_path, 'rb') as fin:
            execution_plan = pickle.load(fin)   # [NOTE]: this obj shared by all processors in memory !!!
        # add some params to execution_plan
        X, Y = get_XY_from_plan_path(plan_path)
        assert X == execution_plan.X and Y == execution_plan.Y, f'X, Y not match: {X}, {Y}, {execution_plan.X}, {execution_plan.Y}'
        causal = execution_plan.da_config.causal
        fob = execution_plan.fob
        if fob == 0:
            f = raw_f
        ...
        for kernel in execution_plan.gpu_kernel_lists[local_rank]:
            kernel.in_ranks = set([local_rank])
        # modify da_config
        if execution_plan.da_config.S != da_config.S:
            execution_plan.da_config.S = da_config.S
        if execution_plan.da_config.Nh != da_config.Nh:
            execution_plan.da_config.Nh = da_config.Nh
        # create streams
        if not is_exist_global_var('streams'):
            streams = []
            # streams.append(torch.cuda.current_stream())
        ...
        # build nccl communicator for collective comms
        ncclcomm_dict = get_global_var('ncclcomm_dict')
        cpu_group_dict = get_global_var('cpu_group_dict')
        # Create gloo group for each pair of ranks
        for n_id in range(PROC_INFO['node_num']):
            for y_id in range(Y):
                key = tuple(range(n_id * local_size + y_id * X, n_id * local_size + (y_id + 1) * X))
                if key not in cpu_group_dict.keys():
                    cpu_group_dict[key] = torch.distributed.new_group(key, backend='gloo')
            for x_id in range(X):
                key = tuple(range(n_id * local_size + x_id, (n_id + 1) * local_size, X))
                if key not in cpu_group_dict.keys():
                    cpu_group_dict[key] = torch.distributed.new_group(key, backend='gloo')
        # Create Row&Col PyNcclCommunicator
        cur_x_id = local_rank % X
        cur_y_id = local_rank // X
        r_key = tuple(range(nodeid * local_size + cur_y_id * X, nodeid * local_size + (cur_y_id + 1) * X))
        c_key = tuple(range(nodeid * local_size + cur_x_id, (nodeid + 1) * local_size, X))
        assert r_key in cpu_group_dict.keys()
        assert c_key in cpu_group_dict.keys()
        if r_key not in ncclcomm_dict.keys():
            cpu_r_group = cpu_group_dict[r_key]
            ncclcomm_dict[r_key] = PyNcclCommunicator(cpu_r_group, device=local_rank)
        if c_key not in ncclcomm_dict.keys():
            cpu_c_group = cpu_group_dict[c_key]
            ncclcomm_dict[c_key] = PyNcclCommunicator(cpu_c_group, device=local_rank)
...
def run_all_inter_attn(args, ncclcomm_global, gloo_global_group):
    ...
    tensor_buf = torch.empty(
        ...
        (bs * MAX_SEQ * max(Nhs) * D * 4) * 3    # KV
      + (bs * MAX_SEQ * max(Nhs) * (D * 2) + (1 * (2 + 1))) * 2, # Q
    ...
    # # [NOTE]: annotate imtermediately
    # # Create gloo group for each pair of local ranks        
    # cpu_group_dict = get_global_var('cpu_group_dict')
    # for node_id in range(node_num):
    #     for local_rank0 in range(local_size):
    #         for local_rank1 in range(local_rank0 + 1, local_size):
    #             key = tuple(sorted([node_id * local_size + local_rank0, node_id * local_size + local_rank1]))
    #             # print(f'rank{rank}, intra gloo key: {key}', flush=True)
    #             # print_rank_0(f'intra gloo key: {key}')
    #             if key not in cpu_group_dict.keys():
    #                 cpu_group_dict[key] = torch.distributed.new_group(key, backend='gloo')
    #             torch.cuda.synchronize()
    ...
    for fob in fobs:
        ...
        for Nh in Nhs:
            for Sqkv in Sqkvs: # S per GPU
                ...
                # # Intra full Execution plan:
                # split_degrees = (da_config.SP[1], da_config.SP[1], 1, 1)
                # map_key = inter_comp_profile_map.get_comp_map_key(da_config, ???, split_degrees)
                # intra_full_attn_config = inter_comp_profile_map.get_value_from_map_key(map_key) # (Y, X, fused, Time)
