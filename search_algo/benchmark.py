...
from search_algo.global_vars import get_global_var, set_global_var
...
def benchmark_orchestrate_bsa(args, raw_f, da_config: Dist_Attn_Config, tensor_buf: torch.Tensor, warmup=11, num_iter=20, log=True, 
    ...
    assert len(exp_configs) == 1, f'Now only one exp_config is allowed in benchmark_orchestrate_bsa !!!'
    for exp_config in exp_configs:
        ...
        Nhs = (nheads, nheads)
        bs = batch_size
        # inter_execution_plan should be None for intra_profiling !!!
        if mode == 'profiling':
            assert SP[0] == 1, 'Only intra exe plans are allowed in profiling mode !!!'
            # Hack an exe plan
        ...
        for inter_comp_plans in inter_comp_plans_dicts:
            ...
            for _, intra_execution_plan in inter_comp_plans.items():
                ...
                for kernel in intra_execution_plan.gpu_kernel_lists[local_rank]:
                    kernel.in_ranks = set([rank])
            # Modify da_config
            if isinstance(inter_execution_plan, Execution_Plan) and hasattr(inter_execution_plan, 'da_config'):
