...
from search_algo.bsa_utils import convert_shape_config_to_str, bsa_is_causal
import regex as re
...
class Dist_Attn_Config():
    def __init__(self, SP, S, Nh, bs, D, causal, hierarchy=1, bsa_config: Optional[BSA_Config] = None):
        ...
        self.hierarchy = hierarchy  # (0, 1) -> (intra, inter)
        self.bsa_config: BSA_Config = bsa_config    # prior than 'causal'
        # Calc total Par_D
        assert self.bsa_config is not None, f'Currently, do not support special cases which cannot be expressed in bsa configuration'
        # self.hierarchy_sp = SP[hierarchy]
        # self.tot_sp = reduce(lambda x,y:x*y, SP)
        # self.S_per_gpu = (S[0] // self.tot_sp, S[1] // self.tot_sp)
        # self.raw_variable_keys = ['SP', 'S', 'Nh', 'bs', 'D', 'hierarchy']   
    ...
    @property
    def raw_variable_keys(self):
        return ['SP', 'S', 'Nh', 'bs', 'D', 'hierarchy'] # [TODO]: whether to add bsa_config?
    
    @property
    def shape_config(self):
        return {'S': self.S, 'Nh': self.Nh, 'bs': self.bs, 'D': self.D}
    ...
    @property
    def tot_Par_D(self):
        tot_Par_D = max(self.tot_sp, self.bsa_config.block_table_raw.shape[0]) # [TODO]: Need to be corrected
        if self.hierarchy == 0 and bsa_is_causal(self.bsa_config):   # [NOTE]: HARDCODE for cases where inter_CP != inter_Par_D
            from search_algo.dense_utils import CP2ParD_map
            tot_Par_D = self.tot_sp * (CP2ParD_map[self.SP[0]] // self.SP[0])
        return tot_Par_D
    ...
    def get_distinction_kv_dict(self, other_da_config: Dist_Attn_Config):
        distinction_dict = {}
        for key in self.raw_variable_keys:
            if getattr(self, key) != getattr(other_da_config, key):
                distinction_dict[key] = getattr(other_da_config, key)
        return distinction_dict
          
    # def get_plan_name(self, fob=1): # [DEPRECATED]
    ...
    def __str__(self):  # [DEPRECATED]
        ret = f'SP={self.SP}, Sg={self.S_per_gpu}, S={self.S}, Nh={self.Nh}, bs={self.bs}, D={self.D}, hierarchy={self.hierarchy}'
        ret = ret.replace(' ', '')
        if self.bsa_config:
...
class Comp_Profile_Map():
    ...
    # def change_current_level(self, new_level: int): # [DEPRECATED]
    #     raise NotImplementedError
        
    def get_comp_time_from_map_key(self, map_key: tuple) -> np.ndarray:
        # print_rank_0(f'self.profile_map.keys(): {self.profile_map.keys()}')
...
class FlashAttn_Profile_Map(Comp_Profile_Map):
    ...
    def get_comp_map_key(self, da_config: Dist_Attn_Config, batch_degrees: list, split_degrees: list, causal: bool = False) -> tuple:
        ...
        Sq_split_norm, Skv_split_norm = Sq_split // S_gcd, Skv_split // S_gcd
        QK_RATIO = f'{Sq_split_norm}/{Skv_split_norm}'
        # assert QK_RATIO in [0.25, 0.5, 1, 2, 4, '3/1', '1/3'], \
        # assert QK_RATIO in ['1/1', '2/1', '1/2', '3/1', '1/3', '4/1', '1/4'], \
        assert max(Sq_split_norm, Skv_split_norm) <= 8, \
...
class Inter_Comp_Profile_Map(Comp_Profile_Map):
    # def __init__(self, dense_profile_map: dict, bsa_profile_map: dict = None):    # [DEPRECATED]
    #     super().__init__()
    #     if dense_profile_map:   # [TODO]: Is this useless?
    ...
    def __init__(self, profile_map: dict = None):
        super().__init__()
        # self.bsa_profile_map = bsa_profile_map  # (hfu, str(time))
        self.profile_map = profile_map  # (hfu, str(time))
        
    def update_inter_bsa_profile(self, intra_bsa_exe_plans_profile):
        self.profile_map = intra_bsa_exe_plans_profile
    ...
    def get_comp_time_from_map_key(self, map_key: tuple) -> np.ndarray:
        if 'bsa' in map_key:    # inter_bsa
            ...
            t[fob] = float(self.profile_map[map_key]['time'])   # s
...
class Dist_Attn_Schedule():
    def __init__(self, da_config: Dist_Attn_Config, m_config: Machine_Config, 
        ...
        '''
        'S' of da_config may be variable.
        '''
    ...
    @property
    def kv_unit_ratio(self):
        Sq_split = self.da_config.S[0] // self.split_degrees[0]
        Skv_split = self.da_config.S[1] // self.split_degrees[1]
        kv_unit_ratio = Skv_split / Sq_split * self.da_config.Nh[1] / self.da_config.Nh[0]  # for crossattn and GQA
        return kv_unit_ratio
    
    @property
    def u_inp_row(self):
        return np.array([1, 2])    # (q), (q, o, do) -> (q, do)

    @property
    def u_inp_col(self):
        return np.array([2, 2]) * self.kv_unit_ratio    # (k, v), (k, v)
    
    @property
    def u_out_row(self):
        return np.array([1, 1])    # (o), (dq)
    
    @property
    def u_out_col(self):
        return np.array([0, 2]) * self.kv_unit_ratio    # (), (dk, dv)
    
    def update_da_config(self, da_config: Dist_Attn_Config):
        self.da_config = da_config
...
# class Search_Engine():  # [DEPRECATED]
...
def create_schedule(da_config, m_config, split_degrees: list, S_map: np.ndarray, schedule_table_func, hierarchy: bool = 1):
    schedule_table = schedule_table_func(split_degrees, S_map, da_config=da_config)
...
# def get_cc_optimal_schedule(da_config, m_config):   # [DEPRECATED]
#     '''
...
# def get_init_schedule_list(da_config, m_config):    # [DEPRECATED]
#     # [NOTE]: 
